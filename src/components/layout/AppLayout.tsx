   ...
   <aside className="hidden md:flex flex-col w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 p-4 sticky top-0 h-screen z-30">
    <div className="flex items-center gap-2 mb-6 px-1">
     <motion.div 
      ...
      whileTap={{ scale: 0.95 }}
      className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center text-white shadow-md shadow-blue-500/20"
     >
      <CheckSquare size={20} />
     </motion.div>
     ...
     {navItems.map((item) => (
      <Link key={item.path} to={item.path}>
       <Button
        variant="ghost"
        className={cn(
         "w-full justify-start gap-2 h-10 rounded-lg transition-all duration-200 group relative overflow-hidden",
    ...
    <div className="mt-auto pt-4 space-y-3 border-t border-slate-100 dark:border-slate-800">
     <Button
      ...
      className="w-full justify-start gap-2 h-10 rounded-lg text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800"
     >
   ...
   <AnimatePresence>
    {isMobileMenuOpen && (
     <motion.div
      ...
      className="md:hidden fixed inset-0 top-[56px] bg-white dark:bg-slate-900 z-40 p-4 flex flex-col"
     >
       ...
       {navItems.map((item) => (
        <Link key={item.path} to={item.path} onClick={() => setIsMobileMenuOpen(false)}>
         <Button
          variant="ghost"
          className={cn(
           "w-full justify-start gap-3 h-12 text-base rounded-xl font-medium",
           location.pathname === item.path 
      ...
      <div className="mt-auto pt-4 border-t border-slate-100 dark:border-slate-800">
       <Button
        variant="outline"
        className="w-full h-12 rounded-xl gap-2 font-medium border-slate-200 dark:border-slate-800"
        onClick={() => {
     ...
     {!isHome && (
      ...
      >
       <Button 
        variant="ghost" 
        onClick={() => navigate(-1)} 
        className="rounded-lg gap-1.5 text-sm text-slate-500 hover:text-slate-900 dark:hover:text-slate-100"
       >
        <ArrowLeft size={16} />
        Back
       </Button>

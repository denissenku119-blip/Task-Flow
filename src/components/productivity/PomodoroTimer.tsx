...
const PRESETS: TimerPreset[] = [
 { id: 'pomodoro', name: 'Classic Pomodoro', mode: {
 work: 25,
 shortBreak: 5,
 longBreak: 15,
 ...
 { id: 'study', name: 'Study Session', mode: {
 work: 45,
 shortBreak: 15,
 longBreak: 20,
 ...
 { id: 'quick', name: 'Quick Focus', mode: {
 work: 15,
 shortBreak: 3,
 longBreak: 10,
 sessionsBeforeLong: 4
 ...
 { id: 'custom', name: 'Custom Timer', mode: {
 work: 0,
 shortBreak: 0,
 longBreak: 0,
...
const PomodoroTimer = () => {
 ...
 mode === 'shortBreak' ? preset.mode.shortBreak * 60 :
 mode === 'longBreak' ? preset.mode.longBreak * 60 :
 (inputValues.hours * 3600) + (inputValues.minutes * 60) + inputValues.seconds;
 const progress = (timeLeft / totalTime) * 100;

 const switchMode = useCallback((nextMode: 'work' | 'shortBreak' | 'longBreak' | 'custom') => {
 setIsActive(false);
 setMode(nextMode);
 if (nextMode === 'work') setTimeLeft(workDuration * 60);
 else if (nextMode === 'shortBreak') setTimeLeft(preset.mode.shortBreak * 60);
 else if (nextMode === 'longBreak') setTimeLeft(preset.mode.longBreak * 60);
 else setTimeLeft((inputValues.hours * 3600) + (inputValues.minutes * 60) + inputValues.seconds);
 }, [workDuration, preset.mode, inputValues]);
 ...
 }, [mode, workDuration, preset.mode, inputValues]);
 ...
 if (newCount % preset.mode.sessionsBeforeLong === 0) {
 ...
 }, [isActive, timeLeft, mode, sessionsCompleted, preset.mode, inputValues]);
 ...
 setTimeLeft((inputValues.hours * 3600) + (inputValues.minutes * 60) + inputValues.seconds);
 };
 ...
 >
 {p.name} ({p.id === 'custom' ? 'Custom' : `${p.mode.work}/${p.mode.shortBreak}`})
 </DropdownMenuItem>
 ))}
 </DropdownMenuContent>
 </DropdownMenu>
 ...
 {Array.from({ length: preset.mode.sessionsBeforeLong }).map((_, i) => (
 <div 
 key={i} 
 className={cn(
 "w-2 h-2 rounded-full transition-all duration-500",
 i < (sessionsCompleted % preset.mode.sessionsBeforeLong) 

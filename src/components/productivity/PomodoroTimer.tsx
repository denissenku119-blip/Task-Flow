    ...
    <div className="flex flex-col items-center">
     <div className="relative w-56 h-56 flex items-center justify-center mb-6">
      <svg className="w-full h-full -rotate-90 transform">
       ...
       <motion.circle
        ...
        animate={{ strokeDashoffset: 653.45 * (1 - progress / 100) }}
     ...
     <div className="flex gap-3 w-full">
      <Button
       ...
       className={cn(
        "flex-1 h-12 rounded-xl text-base font-bold transition-all shadow-md active:scale-95",
        isActive
      ...
      <Button
       ...
       size="icon"
       onClick={handleReset}
       className="h-12 w-12 rounded-xl border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 active:scale-95"
      >
       <RotateCcw size={20} />
      </Button>
     </div>
     ...
     <button
      onClick={openModal}
      className="mt-4 text-xs font-medium text-slate-400 hover:text-blue-600 transition-colors"
     >
   ...
   <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
    <DialogContent className="rounded-2xl max-w-sm">
     ...
     <div className="flex items-end justify-center gap-2 py-2">
      <div className="flex flex-col items-center">
       <Input
        ...
        max={99}
        value={draft.hours}
        onChange={(e) => setDraft((d) => ({ ...d, hours: clampField(e.target.value, 99) }))}
        className="w-20 text-center text-2xl font-bold h-14 rounded-xl tabular-nums"
       />
      ...
      <div className="flex flex-col items-center">
       <Input
        ...
        max={59}
        value={draft.minutes}
        onChange={(e) => setDraft((d) => ({ ...d, minutes: clampField(e.target.value, 59) }))}
        className="w-20 text-center text-2xl font-bold h-14 rounded-xl tabular-nums"
       />
      ...
      <div className="flex flex-col items-center">
       <Input
        ...
        max={59}
        value={draft.seconds}
        onChange={(e) => setDraft((d) => ({ ...d, seconds: clampField(e.target.value, 59) }))}
        className="w-20 text-center text-2xl font-bold h-14 rounded-xl tabular-nums"
       />
     ...
     <DialogFooter className="gap-2">
      ...
      <Button onClick={applyDuration} className="bg-blue-600 hover:bg-blue-700 text-white rounded-xl px-6">
       Apply
      </Button>
     </DialogFooter>
    </DialogContent>
